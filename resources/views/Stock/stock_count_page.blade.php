...
<div class="row page-titles">
    <div class="col-md-5 col-8 align-self-center">
        <h4 class="font-weight-normal">@lang('lang.stock_count') and Price</h4>
    </div>
</div>
                        ...
                        <tbody id="units_table">
                            ...
                            @endphp
                                ...
                                <tr>
                                    ...
                                    {{-- @if(session()->get('user')->role == "Owner") --}}
                                    <td>
                                        <input type="number" class="form-control w-50 stockinput text-black" data-stockinputid="stockinput{{$unit->id}}" id="stockinput{{$unit->id}}" data-id="{{$unit->id}}"value="{{$unit->current_quantity}}">
                                    </td>
                                    {{-- @else
                                    <td>
                                      {{$stockcountt}}
                                    ...
                                    @endif --}}
                                    <td>
                                        <input type="number" class="form-control w-50 spriceinput text-black" data-spriceinputid="spriceinput{{$unit->id}}" id="spriceinput{{$unit->id}}" data-id="{{$unit->id}}"value="{{$unit->order_price}}">
                                    </td>
...
<script>
    ...
    $('#units_table').on('keypress','.spriceinput',function(){
        var keycode= (event.keyCode ? event.keyCode : event.which);
        if(keycode=='13'){
            // var shop_id = $('#shop_id option:selected').val();
            var shop_id = $('#shop_id').val() ?? $('#isshop').val();
            ...
            var unit_id= $(this).data('id');
            var spriceinputid = $(this).data('spriceinputid');
            $.ajax({

                type:'POST',

                url:'{{route('salepriceupdate-ajax')}}',
                ...
                success:function(data){
                    if(data){
                        swal({
                            ...
                            position:'top-end',
                            title:"Success",
                            text:"Sale Price Changed!",
                            button:false,
                            timer:500,
                            icon:"success"
                        });
                        $(`#${spriceinputid}`).addClass("is-valid");
                        $(`#${spriceinputid}`).blur();
                    }
                    else{
                        swal({
                        ...
                        $(`#${spriceinputid}`).addClass("is-invalid");
                    }
                },
                });
        }
